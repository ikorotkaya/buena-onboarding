      ...
      <nav className="flex justify-center" aria-label="Progress">
        <ol role="list" className="space-y-6">
          {ONBOARDING_PAGES.map((step, index) => (
            <li key={step.name}>
              { isStepActive(index) && (
                <Link to={step.slug} className="flex items-start" aria-current="step">
              ...
              { isStepChecked(index) && (
                <Link to={step.slug} className="group">
                  <span className="flex items-start" >
                    ...
                    <span className={`ml-3 text-sm font-medium ${isStepCurrent(index) ? 'text-indigo-600' : 'text-gray-500'}`}>
                      {step.name}
                    </span>
                  </span>
                </Link>
